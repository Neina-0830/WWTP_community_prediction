...
class MyDataset(Dataset):
    ...
    def __init__(self, col=1):
        data1 = np.loadtxt('E:/Desktop/WWTP-机器学习/All_ANN/result/4-fold/ASV/rawdata/Environment_select.csv',delimiter=',',skiprows=1,usecols=range(1,49), dtype=np.float32)
...
for col in range(1,1494):
    for seed in range(0,20):
        ...
        wd = 0.01
        #加载模型       
        net=torch.load(str(col) + 'col-4fold-seed'+ str(seed) +'-10000ep-256bS-0.00001lr-'+ str(wd) +'wd-drop'+ str(drop_P) +'_train_network.pth',map_location='cpu')        
        ...
        seed_torch(seed)
        ##加载test数据
        x_test, y_test = get_testdata(col)
        ...
        #整合结果
        each_test = np.c_[col_result, seed_result, wd_result, drop_P_result, x_test_, y_test_, Test_pred_]
        Test_result = np.r_[Test_result, each_test]
        
        each = np.c_[col,seed,wd,drop_P,MSE_T,R2_T,RI.reshape(1,48),para_bias]   
        para = np.r_[para,each]

np.savetxt('/media/wxl/Run1/lxn/WWTP_ML/results/ASV/analysis/' + 'Test_log-10_select-sigmoid.txt',Test_result, fmt='%.4e', delimiter='\t', newline='\n')        
np.savetxt('/media/wxl/Run1/lxn/WWTP_ML/results/ASV/analysis/' + 'Test_and_parameters-10_select-sigmoid.csv',para, fmt='%.4e', delimiter=',', newline='\n')
